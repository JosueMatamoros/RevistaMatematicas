// Simular carga inicial
            setTimeout(() => {
                document.getElementById('opt-trap-loading').style.display = 'none';
                document.getElementById('opt-trap-app').style.display = 'block';
                initializeOptTrapVisualization();
            }, 10);

            function initializeOptTrapVisualization() {
                ...
                function updateValueTable() {
                    ...
                    let tableHTML = `
                        <table class="opt-trap-table">
                            <thead>
                                <tr>
                                    <th colspan="2">Últimos Valores</th>
                                </tr>
                                <tr>
                                    <th>a</th>
                                    <th>Área(a)</th>
                    ...
                    document.getElementById('opt-trap-values-table').innerHTML = tableHTML;
                }
                
                // Dibujar gráfico del trapecio
                function drawTrapezoidVisualization() {
                    const container = document.getElementById('opt-trap-trapezoid-graph');
                    ...
                    const svg = d3.select('#opt-trap-trapezoid-graph')
                        .append('svg')
                        .attr('width', width)
                        .attr('height', height)
                    ...
                    visualizationGroup.selectAll('.opt-trap-x-grid-line')
                        .data(gridTicks)
                        .enter()
                        .append('line')
                        .attr('class', 'opt-trap-grid-line')
                        .attr('x1', d => xScale(d))
                        .attr('x2', d => xScale(d))
                        .attr('y1', 0)
                        .attr('y2', innerHeight);
                    
                    visualizationGroup.selectAll('.opt-trap-y-grid-line')
                        .data(gridTicks)
                        .enter()
                        .append('line')
                        .attr('class', 'opt-trap-grid-line')
                    ...
                    visualizationGroup.append('g')
                        .attr('class', 'opt-trap-axis opt-trap-axis--x')
                        .attr('transform', `translate(0,${yScale(0)})`)
                        .call(d3.axisBottom(xScale).ticks(10));
                    
                    // Eje Y
                    visualizationGroup.append('g')
                        .attr('class', 'opt-trap-axis opt-trap-axis--y')
                    ...
                    visualizationGroup.selectAll('.opt-trap-vertex')
                    ...
                    visualizationGroup.append('line')
                        ...
                        .attr('stroke', '#333')
                        .attr('stroke-width', 1.2)
                        .attr('marker-start', 'url(#opt-trap-arrow-start)')
                        .attr('marker-end', 'url(#opt-trap-arrow-end)');
                    
                    // Definir marcadores de flecha
                    svg.append('defs').append('marker')
                        .attr('id', 'opt-trap-arrow-start')
                ...
                function drawAreaChart() {
                    const container = document.getElementById('opt-trap-area-graph');
                    const width = container.clientWidth;
                    const height = container.clientHeight;
                    const margin = { top: 25, right: 25, bottom: 35, left: 45 };
                    const innerWidth = width - margin.left - margin.right;
                    ...
                    const svg = d3.select('#opt-trap-area-graph')
                        .append('svg')
                        .attr('width', width)
                        .attr('height', height)
                        .attr('viewBox', `0 0 ${width} ${height}`)
                    ...
                    chartGroup.selectAll('.opt-trap-x-grid-line')
                        .data(xTicks)
                        .enter()
                        .append('line')
                        .attr('class', 'opt-trap-grid-line')
                    ...
                    chartGroup.selectAll('.opt-trap-y-grid-line')
                        .data(yTicks)
                        .enter()
                        .append('line')
                        .attr('class', 'opt-trap-grid-line')
                    ...
                    // Puntos históricos
                    chartGroup.selectAll('.opt-trap-data-point')
                        .data(valueHistory)
                        .enter()
                        .append('circle')
                ...
                document.getElementById('opt-trap-a-slider').addEventListener('input', function(e) {
                    currentA = parseFloat(e.target.value);
                    document.getElementById('opt-trap-a-value').textContent = currentA.toFixed(2);
