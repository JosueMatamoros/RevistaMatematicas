"use client";
import { FaHome, FaBook, FaUserEdit, FaInfoCircle } from "react-icons/fa";
import { FaPencil, FaArrowDown19 } from "react-icons/fa6";
export default function NavsComponent({ isSidebarOpen}) {
  return (
    <nav className="w-full rounded-md border bg-white shadow-md px-6 py-3 relative">
      <ul className={`flex ${
        isSidebarOpen ? "justify-start" : "justify-center"
      } items-center space-x-8 text-base`}
      >
        {/* Inicio */}
        ...
        <li className="relative group">
          ...
          <ul className="absolute left-0 mt-2 z-10 min-w-[220px] rounded-md bg-white shadow-lg p-1.5 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
            {[
              ...
              <li
                key={item}
                className="cursor-pointer text-slate-800 w-full text-base rounded-md p-2 transition-all hover:bg-tec-blue-primary hover:text-white"
              >
                {item}
              </li>
            ))}
        ...
        <li className="relative group">
          ...
          <ul className="absolute left-0 mt-2 z-10 min-w-[220px] rounded-md bg-white shadow-lg p-1.5 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
            {["Libros", "Materiales revisados", "Materiales sin revisión"].map((item) => (
              <li
                key={item}
                className="cursor-pointer text-slate-800 w-full text-base rounded-md p-2 transition-all hover:bg-tec-blue-primary hover:text-white"
              >
